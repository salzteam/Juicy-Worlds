...
const getPromo = (queryParams, hostApi) => {
  return new Promise((resolve, reject) => {
    ...
    let query = `select pr.code, pr.discount, p.product_name, p.price, c.category_name, p.image from promos pr left join products p on pr.product_id = p.id join categories c on p.category_id = c.id`;
    let queryLimit = "";
